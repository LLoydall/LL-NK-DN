...
export function parseWorkbookBuffer(buffer: Buffer, sourceLabel: string): LoadedWorkbook {
  ...
  return { documents, sheetCount: workbook.SheetNames.length, sourceLabel };
}
 ...
 */
export function loadUploadedWorkbook(buffer: Buffer, originalname: string): LoadedWorkbook {
...
export async function loadWorkbook(inputPath: string): Promise<LoadedWorkbook> {
  ...
  return parseWorkbookBuffer(await fs.readFile(resolved), path.relative(REPO_ROOT, resolved));
}
